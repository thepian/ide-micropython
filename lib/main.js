...
class MicroPythonLanguageClient extends AutoLanguageClient {
  getGrammarScopes() {
    return ["source.upython", "upython"];
  }
  ...
  getRootConfigurationKey() {
    return "ide-micropython";
  }

  activate() {
    // Remove deprecated option
    atom.config.unset("ide-micropython.pylsPath");
    super.activate();
  }
  ...
  async startServerProcess(projectPath) {
    ...
    childProcess.on("error", err => {
      const description =
        err.code == "ENOENT"
          ? `No MicroPython interpreter found at \`${python}\`.`
          : `Could not spawn the Python interpreter \`${python}\`.`;
      atom.notifications.addError(
        "`ide-micropython` could not launch your MicroPython runtime.",
        {
          dismissable: true,
...
module.exports = new MicroPythonLanguageClient();
